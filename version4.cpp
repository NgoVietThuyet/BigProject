...
void Step()
{
    ...
    switch (direction) {
        case 'r':
            fileHead = "image//head_right.png";
            stepX = step;
            break;
        case 'l':
            fileHead = "image//head_left.png";
            stepX = -step;
            break;
        case 'u':
            fileHead = "image//head_up.png";
            stepY = -step;
            break;
        case 'd':
            fileHead = "image//head_down.png";
            stepY = step;
            break;
    }
}
    ...
    for(int i = 0; i < snake.size(); i++)
     {
         ...
         if( i != 0)
         {
             string fileBody = "image//body.png";
              SDL_Texture* head = loadTexture(fileBody);
                SDL_Rect dstRect = { snake[i].x*20  ,snake[i].y*20 , 25, 25};
         ...
         else
         {
              //drawRect(snake[i],RED);
                SDL_Texture* head = loadTexture(fileHead);
                SDL_Rect dstRect = { snake[i].x*20 ,snake[i].y*20 , 25, 25 };
                SDL_RenderCopy(renderer, head, NULL, &dstRect);
     ...
     if( COUNTER % 6 == 0)
     {
        ...
        apple = "image//greenApple.png";
        SDL_Texture* head = loadTexture(apple);
        SDL_Rect dstRect = { smallball.x*20 ,smallball.y*20 , 30, 30};
        SDL_RenderCopy(renderer, head, NULL, &dstRect);
